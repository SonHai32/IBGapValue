...
namespace IBGapValue.Services.Impl
{
    ...
    public class EWrapperImpl : EWrapper
    {
        ...
        private IBMessageServiceImp _iBMessageService => Singleton<IBMessageServiceImp>.Instance;
        //! [socket_declare]
        ...
        public virtual void error(int id, int errorCode, string errorMsg, string advancedOrderRejectJson)
        {
            ...
            _iBMessageService.NotifyReceivedErrorMessage(id);
        }
        ...
        {
            Console.WriteLine(bar.Time + bar.Open);
            _iBMessageService.SendMessage(reqId, bar);
            _iBMessageService.CurrentReqBarId = reqId;
        }
        ...
        {
            _iBMessageService.NotifyReceivedMarketScanner(reqId, contractDetails.Contract);
        }
        ...
        {
            _iBMessageService.NotifyReceivedMarketScannerEnd();
        }
        ...
        {
            _iBMessageService.EndMessage(reqId);
            if (_iBMessageService.CurrentReqBarId == reqId)
            {
                _iBMessageService.ReqBarIdCount++;
            }
            if (_iBMessageService.ReqBarIdCount == 7)
            {
                _iBMessageService.NotifyReqBarDone(reqId);
                _iBMessageService.CurrentReqBarId = -1;
            }
        }
